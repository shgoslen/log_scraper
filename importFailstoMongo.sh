#!/usr/bin/sh
# Take the CSV files and further process them. Produce a document that can be
# added to the Mongo DB so that searches and other stats can be performed
# The records is jobid,seq,platform type,testcase,software version??,error string
...
outFile="/auto/performance/tools/failures/csv/jobFailCat_$ymd.csv"
#touch $outFile
...
for f in $fls; do
        ...
        if [ $rslt -eq 0 ]; then
                ...
                awk -f /auto/performance/tools/failures/getFailCat.awk $reportfile >> $outFile
